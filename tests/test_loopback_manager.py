...
class TestBuildArgv:
    ...
    def test_capture_channelmix_upmix_off_8ch(self, media_spec: LoopbackSpec) -> None:
        """8ch captures must set channelmix.upmix=false so a stereo source is
        not expanded into correlated copies of FL/FR on the 7.1-extra channels —
        ...
        channelmix.disable would be wrong here (it kills the 2↔8ch adaptation
        and the node volume entirely)."""
        argv = _build_pw_loopback_argv(media_spec)
        assert "channelmix.upmix=false" in argv[1]
        assert "channelmix.disable=true" not in argv[1]

    def test_capture_channelmix_default_2ch(self, chat_spec: LoopbackSpec) -> None:
        """2ch captures (Chat) get no channelmix.upmix=off override."""
        argv = _build_pw_loopback_argv(chat_spec)
        assert "channelmix.upmix=false" not in argv[1]
